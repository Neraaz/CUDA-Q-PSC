...
def vqe(elem1,elem2,d):
    ...
    def cost(theta):
        ...
        spin_ham_list = spin_ham.distribute_terms(10)
        process = []
        qpu_count = 0
        exp_val = 0
        for term in spin_ham_list:
            process.append(cudaq.observe_async(kernel, term, qubit_count, electron_count,
                                theta, qpu_id=qpu_count % num_qpus))
            qpu_count += 1

        for proc in process:
            exp_val += proc.get().expectation()
        return exp_val
