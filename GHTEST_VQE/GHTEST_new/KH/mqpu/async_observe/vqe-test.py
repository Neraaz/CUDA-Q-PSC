...
def vqe(elem1,elem2,d):
    ...
    def cost(theta):
        # We need to split hamiltonian and compute expectation values on different GPUs, sum, and return 
        spin_ham_list = spin_ham.distribute_terms(2)
        calc0 = cudaq.observe_async(kernel, spin_ham_list[0], qubit_count, electron_count,
                                theta, qpu_id=0)
        calc1 = cudaq.observe_async(kernel, spin_ham_list[1], qubit_count, electron_count,
                                theta, qpu_id=1)
        exp_val = calc0.get().expectation() + calc1.get().expectation()
        return exp_val
